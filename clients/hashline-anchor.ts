 ...
 *
 * The two assignments differ, and the difference is not benign. Where a line's
 * canonical content is unique in the file its anchor is its content's base
 * index and both sides agree. Where it is NOT unique — `}`, a blank line,
 * `continue;`, a bare comment-close line — the anchor comes from probing, whose
 * result depends on every line before it, so an edit anywhere upstream
 * repermutes those anchors among the identical lines. A store-carried anchor
 * then still matches EXACTLY ONE line under recomputation: a confident,
 * unique, WRONG answer.
 *
 * Measured against `pi-hashline-edit-pro@8f027ca1`'s own `mapStableHashes`
 * over 56 real `clients/*.ts` files with one simulated edit each: up to 56 of
 * ~19400 anchors resolved to the wrong line, every one of them a
 * duplicate-content line, and the worst were off by 7+ lines. Those wrong
 * lines became a wrong range handed to `readGuard.checkEdit`,
 * `cacheManager.addModifiedRange` and the `touched_lines_detected` record.
 *
 * So `resolveHashlineAnchor` answers ONLY when the matched line's canonical
 * content occurs exactly once in the file — the case where our recomputation
 * provably agrees with the store. Everything else is `content_not_unique`:
 * unresolved, which is a report and never a block. pi-lens's uncertainty about
 * a third-party tool's addressing is not evidence that the agent's edit is
 * wrong. An unresolved anchor degrades to "mutation recognized, lines unknown"
 * — the same state a host `edit` with an unknown schema reaches on master — so
 * the file still gets its turn-state entry (the whole file as its modified
 * range), its deferred format and its change-log receipt.
 *
 * This costs coverage: roughly a third of the lines in a typical TypeScript
 * file are blank or a bare brace. Trading those for a guaranteed-correct
 * answer on the rest is the deliberate choice — a wrong line range is worse
 * than no line range, because the guard acts on it.
 */
