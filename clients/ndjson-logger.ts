...
/** A `createNdjsonLogger` handle whose construction was deferred (see `createLazyNdjsonLogger`). */
export interface LazyNdjsonLogger extends NdjsonLogger {
	/** The absolute path writes land at — resolves the same options the writer itself was (or will be) built from. */
	getFilePath(): string;
	/** Test-only: drop the memoized writer so the next call re-resolves `resolveOptions()`. */
	_resetForTests(): void;
}

/**
 * Build an `NdjsonLogger` whose underlying `createNdjsonLogger` instance —
 * and therefore its `filePath`/`maxBytes` resolution — is not constructed
 * until the first real call (`log`/`append`/`truncate`/`flush`/`flushSync`/
 * `getFilePath`).
 *
 * #2506: every `*-logger.ts` module in this codebase used to resolve its
 * `filePath` (via `getGlobalPiLensDir()`, which reads `PI_LENS_HOME`) into a
 * top-level `const`, evaluated at MODULE IMPORT time. Whichever process
 * imported the module first froze every later write to whatever
 * `PI_LENS_HOME` resolved to at THAT moment — including vitest's
 * `globalSetup`, which runs before `vitest-setup.ts`'s per-worker
 * `PI_LENS_HOME` override is ever set (confirmed 2026-09-02: a canary log
 * showed `PI_LENS_HOME=undefined` during `globalSetup`, i.e. `os.homedir()`).
 * Production code is unaffected by that ONE freeze (the process that runs
 * `globalSetup` never itself writes there), but the shape is the same one
 * `env-utils.ts`'s `lazyEnvNumber` doc comment already forbids for a plain
 * config number: "importing a module must never have a side effect on
 * process env at load time." A file path derived from an overridable env var
 * is the identical hazard with a worse blast radius — a write, not a stale
 * read.
 *
 * This is the file-path/writer-construction analogue of that same
 * `_resetForTests` convention, built on the existing `createNdjsonLogger`
 * (not a parallel writer implementation): `resolveOptions()` is called
 * exactly once, on the first real call, and its `filePath` is still a plain
 * STRING by the time it reaches `createNdjsonLogger` — so the writer
 * self-registers into `getRegisteredLogFiles()` exactly as a static-path
 * logger always has (see that function's doc comment in this file), keeping
 * `log-cleanup.ts`'s auto-derived rotation coverage intact. A raw lazy
 * `() => string` `filePath` passed straight to `createNdjsonLogger` would
 * NOT do this — see `getRegisteredLogFiles()`'s doc comment for why a lazy
 * resolver is deliberately excluded from that registry.
 */
export function createLazyNdjsonLogger(
	resolveOptions: () => NdjsonLoggerOptions & { filePath: string },
): LazyNdjsonLogger {
	let instance: { logger: NdjsonLogger; filePath: string } | undefined;
	function ensure(): { logger: NdjsonLogger; filePath: string } {
		if (!instance) {
			const options = resolveOptions();
			instance = {
				logger: createNdjsonLogger(options),
				filePath: normalizeLogPath(options.filePath),
			};
		}
		return instance;
	}
	return {
		log(obj: unknown): void {
			ensure().logger.log(obj);
		},
		append(line: string): void {
			ensure().logger.append(line);
		},
		truncate(): void {
			ensure().logger.truncate();
		},
		async flush(): Promise<void> {
			await ensure().logger.flush();
		},
		flushSync(): void {
			ensure().logger.flushSync();
		},
		getFilePath(): string {
