...
export const EXEMPT_SESSION_STATE_FILES: Readonly<Record<string, string>> = {
	...
	"probe-home-state.ts":
		"#2506: the globalThis-keyed probe-home-redirect event, the same shape as process-singletons.ts's container just below — a PROCESS boundary, not a session one. getGlobalPiLensDir()'s redirect decision is fixed by cwd/env at process start and cannot recur within the same process once it has fired, so a session_start reset would only hide the fact from every session after the first in the same probe process. No module-scope binding at all (not even a Symbol.for constant): file-utils.ts and degradation-ledger.ts are already mutually reachable through the existing extension-log.ts/latency-logger.ts/safe-spawn.js cycle, and log-cleanup.ts's eager top-level getGlobalPiLensDir() call can reach this module's exported functions before this module's own body has run, so even a module-scope `const` key is TDZ-unsafe here — every Symbol.for call is inlined per invocation instead.",
...
export const SESSION_STATE_SYMBOL_COUNTS: Readonly<Record<string, number>> = {
	...
	// #2506: no module-scope binding at all, not even a Symbol.for constant
	// (every call recomputes its key inline) — the scan sees no container here.
