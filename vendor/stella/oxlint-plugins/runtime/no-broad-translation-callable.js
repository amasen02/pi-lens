...
const isBroadTranslatorName = (node) => isIdentifierNamed(node, "getTranslator") ||
    isIdentifierNamed(node, "getTranslations") ||
    isIdentifierNamed(node, "useTranslations");
const isTranslationTypeModule = (source) => source.endsWith("/i18n/types") || source.endsWith("/i18n/utils");
...
const unwrapTypeAnnotation = (node) => node?.type === "TSTypeAnnotation" ? node.typeAnnotation : node;
...
const isQualifiedNamespaceMember = (node, namespaceVariables, memberNames, resolveVariable) => {
...
const isBroadTranslatorReturnType = (node, broadTranslatorTypeNames, broadTranslatorNamespaceVariables, resolveVariable) => {
    if (node.type !== "TSTypeReference" ||
        !isIdentifierNamed(node.typeName, "ReturnType")) {
        return false;
    }
    const queriedType = node.typeArguments?.params?.at(0);
    return (queriedType?.type === "TSTypeQuery" &&
        ...
        ((queriedType.exprName?.type === "Identifier" &&
            ...
            isQualifiedNamespaceMember(queriedType.exprName, broadTranslatorNamespaceVariables, broadTranslatorMemberNames, resolveVariable)));
};
const isCanonicalTranslationKeyDerivation = (node) => {
    ...
    return (returnTypeArguments?.type === "TSTypeParameterInstantiation" &&
    ...
    rules: {
        "no-broad-translation-callable": {
            meta: {
                type: "problem",
                messages: {
                    broadAliasExport: "Do not export an alias of the full TranslationKey union; consumers could reintroduce a broad translation callable under a hidden name.",
                    broadCallable: "Do not pass the full use-intl translator across a helper boundary; translate fixed keys at the UI call site, pass plain strings, or expose a narrow key union.",
                },
            },
            createOnce(context) {
                ...
                const broadReturnTypeCandidates = new Array();
                const exportedAliasNodes = new Map();
                const localExportCandidates = new Array();
                const renamedReexportCandidates = new Array();
                ...
                const isBroadKeyTypeReference = (node) => {
                    if (node?.type !== "TSTypeReference") {
                        return false;
                    }
                    ...
                    return isQualifiedNamespaceMember(node.typeName, broadKeyNamespaceVariables, translationKeyTypeNames, resolveVariable);
                };
                const hasTranslationKeyParameter = (node) => (node.params ?? []).some((parameter) => isBroadKeyTypeReference(unwrapTypeAnnotation(parameter.typeAnnotation)));
                ...
                return {
                    ...
                    "Program:exit"() {
                        ...
                        for (const candidate of broadReturnTypeCandidates) {
                            if (!isCanonicalTranslationKeyDerivation(candidate) &&
                                isBroadTranslatorReturnType(candidate, broadTranslatorTypeNames, broadTranslatorNamespaceVariables, resolveVariable)) {
                    ...
                    ExportNamedDeclaration(node) {
                        ...
                        for (const specifier of node.specifiers ?? []) {
                            if (typeof source === "string" &&
                                isTranslationTypeModule(source)) {
                                if (isIdentifierNamed(specifier.local, "TranslationKey") &&
