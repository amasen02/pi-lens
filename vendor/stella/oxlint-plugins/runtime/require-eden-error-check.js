...
// cycles.
import { getImportedName, getPropertyName, isAstNode, isIdentifier, isStringLiteral, unwrapExpression, } from "./utils.js";
    ...
    rules: {
        "require-eden-error-check": {
            meta: {
                type: "problem",
                messages: {
                    requireEdenErrorCheck: "Eden response consumed via '.{{method}}()'. api.* calls resolve " +
                        ...
                        "message) instead.",
                    requireEdenErrorCheckDiscarded: "Eden call result discarded via '{{form}}'. api.* calls resolve " +
                    ...
                    requireEdenAssignedErrorCheck: "This assigned Eden response can be consumed or leave scope " +
            ...
            createOnce(context) {
                ...
                const pendingEdenBindings = new Map();
                ...
                const resolveVariable = (identifierNode) => {
                    const findVariable = (scope) => scope?.set.get(identifierNode.name) ??
                ...
                const isApiImportVariable = (variable) => variable !== null &&
                    variable.defs.some((def) => {
                        ...
                        if (!isAstNode(def.parent) ||
                            def.parent.type !== "ImportDeclaration") {
                            return false;
                        }
                        return (isStringLiteral(def.parent.source) &&
                ...
                const isGenuineApiRoot = (root, visited = new Set(), depth = 0) => {
                ...
                const awaitedTerminalEdenCall = (node) => {
                ...
                const containsAwaitedTerminalEdenValue = (node) => {
                ...
                const hasNonEdenPath = (node) => {
                    ...
                    return (hasNonEdenPath(expression.consequent) ||
                        hasNonEdenPath(expression.alternate));
                };
                const isApprovedResponseAdapter = (node) => {
                ...
                const isBindingIdentifier = (node, binding) => {
                    const expression = unwrapExpression(node);
                    return (isIdentifier(expression) && resolveVariable(expression) === binding);
                };
                const hasUnhandled = (states) => states.has("unhandled");
                const handledStates = (states) => {
                    const next = new Set();
                ...
                const containsBindingReference = (node, binding, seen = new Set()) => {
                    ...
                    for (const [key, value] of Object.entries(node)) {
                        ...
                        if (Array.isArray(value)) {
                            if (value.some((child) => containsBindingReference(child, binding, seen))) {
                                return true;
                            }
                            continue;
                        }
                ...
                const patternProperties = (pattern) => {
                ...
                const analyzeExpression = (node, states, binding) => {
                    const expression = unwrapExpression(node);
                    if (!isAstNode(expression)) {
                    ...
                    if (expression.type === "MemberExpression") {
                        if (isBindingIdentifier(expression.object, binding)) {
                            const property = expression.computed === true
                    ...
                    if (expression.type === "CallExpression" ||
                        ...
                        const calleeOutcome = analyzeExpression(expression.callee, current, binding);
                    ...
                    if (expression.type === "ConditionalExpression") {
                        ...
                        const consequent = analyzeExpression(expression.consequent, test.states, binding);
                        const alternate = analyzeExpression(expression.alternate, test.states, binding);
                        return {
                    ...
                    if (expression.type === "LogicalExpression") {
                        ...
                        const right = analyzeExpression(expression.right, left.states, binding);
                        return {
                    ...
                    if (expression.type === "AssignmentExpression") {
                        ...
                        const left = analyzeExpression(expression.left, right.states, binding);
                        return {
                            states: left.states,
                ...
                const analyzeStatements = (statements, states, binding) => {
                    let current = new Set(states);
                    let unsafe = false;
                    let unsafeExit;
                ...
                const analyzeDeclarators = (declarations, states, binding) => {
                    let current = new Set(states);
                    let unsafe = false;
                    for (const declaration of declarations) {
                        ...
                        const outcome = analyzeExpression(declaration.init, current, binding);
                        current = outcome.states;
                        unsafe ||= outcome.unsafe;
                    }
                    return { states: current, unsafe };
                };
                const analyzeStatement = (node, states, binding) => {
                    if (!isAstNode(node)) {
                        return { states: new Set(states), unsafe: false };
                    }
                    if (node.type === "BlockStatement") {
                        return analyzeStatements(Array.isArray(node.body) ? node.body : [], states, binding);
                    }
                    if (node.type === "ExpressionStatement") {
                        return analyzeExpression(node.expression, states, binding);
                    }
                    if (node.type === "VariableDeclaration") {
                        return analyzeDeclarators(Array.isArray(node.declarations) ? node.declarations : [], states, binding);
                    }
                    if (node.type === "ReturnStatement" || node.type === "ThrowStatement") {
                        ...
                        return {
                            ...
                            unsafeExit: argument.unsafeExit ??
                                (exitsUnhandled &&
                    ...
                    if (node.type === "IfStatement") {
                        const nullComparison = (() => {
                            ...
                            const comparesBindingToNull = (isBindingIdentifier(left, binding) &&
                                isAstNode(right) &&
                                right.type === "Literal" &&
                                right.value === null) ||
                        ...
                        if (nullComparison !== null) {
                            ...
                            const consequentStates = nullComparison === "absent" ? absentStates : presentStates;
                            const alternateStates = nullComparison === "absent" ? presentStates : absentStates;
                            const consequent = analyzeStatement(node.consequent, consequentStates, binding);
                        ...
                        const consequent = analyzeStatement(node.consequent, test.states, binding);
                        const alternate = isAstNode(node.alternate)
                    ...
                    if (node.type === "SwitchStatement") {
                        const discriminant = analyzeExpression(node.discriminant, states, binding);
                        const cases = Array.isArray(node.cases) ? node.cases : [];
                        const hasDefault = cases.some((switchCase) => isAstNode(switchCase) && switchCase.test === null);
                        const branches = hasDefault
                            ? []
                        ...
                        for (const switchCase of cases) {
                            ...
                            const test = analyzeExpression(switchCase.test, discriminant.states, binding);
                            const branch = analyzeStatements(Array.isArray(switchCase.consequent)
                    ...
                    if (node.type === "TryStatement") {
                        ...
                        const finalizerInput = combined.unsafeExit
                            ...
                            : combined.states;
                        const finalized = analyzeStatement(node.finalizer, finalizerInput, binding);
                        const pendingExitHandled = combined.unsafeExit !== undefined &&
                            !hasUnhandled(finalized.states);
                        return {
                            states: finalized.states,
                            unsafe: (combined.unsafe && !pendingExitHandled) || finalized.unsafe,
                ...
                const analyzeContinuation = (node, binding, canBeAbsent) => {
                    let current = isAstNode(node) ? node : null;
                    let states = new Set(["unhandled"]);
                    if (canBeAbsent) {
                        states.add("absent");
                    }
                    let unsafe = false;
                    ...
                    if (current?.type === "VariableDeclarator" &&
                        ...
                        const siblings = analyzeDeclarators(index === -1 ? [] : declarations.slice(index + 1), states, binding);
                    ...
                    while (current !== null) {
                        ...
                        if (parent !== null &&
                            ...
                            if (index !== -1) {
                                const outcome = analyzeStatements(parent.body.slice(index + 1), states, binding);
                        ...
                        if (parent?.type === "TryStatement" &&
                            ...
                            const finalized = analyzeStatement(parent.finalizer, states, binding);
                            states = finalized.states;
                ...
                const reportAssignedResult = (node) => {
                    context.report({
                        node,
                ...
                return {
                    ...
                    VariableDeclarator(node) {
                        if (terminalEdenCall(node.init) !== null &&
                    ...
                    "Program:exit"() {
                        ...
                        for (const result of assignedResults) {
                            const outcome = analyzeContinuation(result.node, result.binding, result.canBeAbsent);
