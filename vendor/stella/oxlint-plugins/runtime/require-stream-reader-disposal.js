...
import { getPropertyName, isAstNode, isIdentifier, isStringLiteral, unwrapExpression, } from "./utils.js";
...
const nearestFunctionOrProgram = (node) => {
...
const visitNodes = (node, root, visitor, seen = new Set()) => {
...
const callWithinExpressionStatement = (statement, predicate) => {
    if (!isAstNode(statement) || statement.type !== "ExpressionStatement") {
        return false;
    }
    const containsUnconditionalCall = (node) => {
...
const abruptCompletionTarget = (node) => {
...
const nearestLoopWithin = (node, boundary) => {
...
const breakExitsLoop = (node, loop) => {
    ...
    rules: {
        [RULE_NAME]: {
            meta: {
                type: "problem",
                messages: {
                    cancelBeforeRelease: "This ReadableStream reader can stop before EOF. Cancel it before releasing its lock so the producer is not left active.",
                    releaseInFinally: "Release this ReadableStream reader's lock in a finally block, or explicitly return the reader to transfer ownership.",
                },
            },
            createOnce(context) {
                const acquisitions = [];
                const isEstreeIdentifier = (node) => isIdentifier(node) && Array.isArray(node.range);
                ...
                const namedType = (node, name) => {
                    ...
                    if (typeNode.type === "TSUnionType" && Array.isArray(typeNode.types)) {
                        const meaningfulTypes = typeNode.types.filter((part) => isAstNode(part) &&
                ...
                const responseBodyInitializer = (variable) => {
                    for (const definition of variable.defs) {
                        if (definition.type !== "Variable" ||
                            !isAstNode(definition.node) ||
                            definition.node.type !== "VariableDeclarator" ||
                ...
                const isGlobalConstructor = (node, name) => {
                    const expression = unwrapValue(node);
                    ...
                    return ["globalThis", "self", "window"].some((host) => isGlobalReference(callee.object, host));
                };
                ...
                const isResponseExpression = (node, visited = new Set()) => {
                    const expression = unwrapValue(node);
                    if (expression === null) {
                        return false;
                    }
                    ...
                    return (initializer !== null && isResponseExpression(initializer, visited));
                };
                const isReadableStreamExpression = (node, visited = new Set()) => {
                    const expression = unwrapValue(node);
                    if (expression === null) {
                        return false;
                    }
                    ...
                    if (expression.type === "MemberExpression") {
                        return (staticMemberName(expression) === "body" &&
                ...
                const canonicalBinding = (node, visited = new Set()) => {
                    const expression = unwrapValue(node);
                    if (!isEstreeIdentifier(expression)) {
                        return null;
                    }
                ...
                const readerMethodCall = (node, method, binding) => {
                    const matched = memberCall(node, method);
                    return (matched !== null && canonicalBinding(matched.object) === binding);
                };
                const containsReaderMethod = (node, method, binding) => {
                ...
                const acquisitionOwner = (node) => {
                    ...
                    if (parent?.type === "VariableDeclarator" &&
                        ...
                        return {
                            ...
                            stable: declaration?.type === "VariableDeclaration" &&
                ...
                const methodStatementIndexes = (block, method, binding) => {
                    if (!isAstNode(block) ||
                    ...
                    for (const [index, statement] of block.body.entries()) {
                        if (callWithinExpressionStatement(statement, (candidate) => readerMethodCall(candidate, method, binding))) {
                            indexes.push(index);
                        }
                    }
                    return indexes;
                };
                const handledCancelStatement = (statement, binding) => {
                    if (!isAstNode(statement) || statement.type !== "ExpressionStatement") {
                        return false;
                    }
                ...
                const finalizerCleanup = (block, binding) => {
                    if (!isAstNode(block) ||
                        block.type !== "BlockStatement" ||
                        !Array.isArray(block.body)) {
                    ...
                    for (const statement of block.body) {
                        if (callWithinExpressionStatement(statement, (candidate) => readerMethodCall(candidate, "releaseLock", binding))) {
                            return {
                                cancelBeforeRelease: handledCancel,
                                releases: true,
                            };
                        }
                        if (callWithinExpressionStatement(statement, (candidate) => readerMethodCall(candidate, "cancel", binding))) {
                        ...
                        if (isAstNode(statement) &&
                            ...
                            if (nested.releases) {
                                const tryCancels = methodStatementIndexes(statement.block, "cancel", binding)
                                    .length > 0;
                                return {
                                    cancelBeforeRelease: handledCancel || tryCancels || nested.cancelBeforeRelease,
                                    releases: true,
                                };
                            }
                        }
                ...
                const tryCoversAcquisition = (tryNode, acquisition, binding) => {
                ...
                const hasTransferredBinding = (acquisition, binding) => {
                    ...
                    visitNodes(acquisition.root, acquisition.root, (node) => {
                        if ((node.type === "ReturnStatement" && node !== finalStatement) ||
                            node.type === "ThrowStatement") {
                            hasCompetingExit = true;
                        }
                        if (node.type === "CallExpression" &&
                            ["cancel", "read", "releaseLock"].some((method) => readerMethodCall(node, method, binding))) {
                            used = true;
                        }
                    });
                    return !hasCompetingExit && !used;
                };
                const readResultKind = (identifier, binding) => {
                    const variable = resolveVariable(identifier);
                    if (variable === null) {
                        return null;
                    }
                    if (variable.references.some((reference) => reference.init !== true && reference.isWrite?.() === true)) {
                        return null;
                    }
                    for (const definition of variable.defs) {
                        if (definition.type !== "Variable" ||
                        ...
                        if (isAstNode(definition.node.id) &&
                            definition.node.id.type === "ObjectPattern" &&
                            Array.isArray(definition.node.id.properties) &&
                            definition.node.id.properties.some((property) => isAstNode(property) &&
                                property.type === "Property" &&
                ...
                const isDoneGuard = (node, binding) => {
                    const expression = unwrapValue(node);
                    if (isEstreeIdentifier(expression)) {
                    ...
                    if (expression?.type === "MemberExpression" &&
                        ...
                        return (isEstreeIdentifier(object) &&
                            readResultKind(object, binding) === "result");
                    }
                    if (expression?.type === "BinaryExpression" &&
                        (expression.operator === "===" || expression.operator === "==")) {
                        return ((isDoneGuard(expression.left, binding) &&
                ...
                const dominatedByDoneGuard = (node, binding) => {
                    let child = node;
                ...
                const hasPriorCancel = (node, binding) => {
                    ...
                    return site.block.body
                        .slice(0, index)
                        .some((statement) => callWithinExpressionStatement(statement, (candidate) => readerMethodCall(candidate, "cancel", binding)));
                };
                const catchCancels = (tryNode, binding) => {
                    if (!isAstNode(tryNode.handler) || !isAstNode(tryNode.handler.body)) {
                        return false;
                    }
                    return (methodStatementIndexes(tryNode.handler.body, "cancel", binding)
                        .at(0) === 0);
                };
                const trailingCancelCoversTopLevelReads = (tryNode, binding) => {
                    if (!isAstNode(tryNode.block) ||
                    ...
                    return methodStatementIndexes(block, "cancel", binding).some((index) => index === lastReadIndex + 1);
                };
                const readPathsRequireFinalCancel = (tryNode, binding) => {
                    ...
                    for (const loop of readLoops) {
                        ...
                        visitNodes(loop, loop, (node) => {
                            ...
                            if (node.type === "AwaitExpression") {
                                ...
                                if (!dominatedByDoneGuard(node, binding) &&
                ...
                return {
                    ...
                    "Program:exit"() {
                        for (const acquisition of acquisitions) {
                            ...
                            if (!cleanup.cancelBeforeRelease &&
