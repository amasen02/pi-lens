...
const isManagedSpecifier = (specifier) => MANAGED_NAMESPACES.some((namespace) => specifier === namespace || specifier.startsWith(`${namespace}/`));
...
const reportInvalidImport = (context, source) => {
...
const reportLeafReexport = (context, source) => {
    const specifier = stringLiteralValue(source);
    if (source === null ||
        specifier === undefined ||
        !isManagedSpecifier(specifier)) {
    ...
    rules: {
        "no-facade-imports": {
            meta: {
                type: "problem",
                messages: {
                    facadeImport: "Import an approved owning leaf instead of {{specifier}}.",
                    leafReexport: "Do not re-export {{specifier}}; consumers must import its owning leaf directly.",
                },
                schema: [],
            },
            createOnce(context) {
