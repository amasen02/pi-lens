...
import { getCalleeName, getPropertyName, isIdentifier, isStringLiteral, } from "./utils.js";
...
const checkObjectExpression = (context, objectNode, contextLabel) => {
    for (const prop of objectNode.properties) {
        ...
        const hasDynamicComputedKey = prop.computed === true && !isStringLiteral(prop.key);
    ...
    rules: {
        "no-secret-in-log-sink": {
            meta: {
                type: "problem",
                messages: {
                    secretInSink: "Do not pass '{{name}}' to {{sink}}. Sinks (JSON.stringify, analytics, Sentry, Error constructors) may serialize the value into logs, telemetry, or response bodies. Strip the field, mask it (maskApiKey), or use a structural error tag.",
                },
            },
            createOnce(context) {
                return {
