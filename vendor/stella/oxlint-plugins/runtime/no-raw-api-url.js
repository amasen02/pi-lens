...
import { filenameForContext, getPropertyName, isAstNode, isIdentifier, isMemberAccess, isStringLiteral, } from "./utils.js";
...
const V1_PATH = /^\/v1(?:\/|$)/u;
const API_ENV_KEYS = ["VITE_API_URL", "VITE_BROWSER_API_URL"];
const API_ENV_OWNERS = [
    "apps/web/src/env.ts",
    "apps/web/src/lib/api-origins.ts",
];
const isDirectApiEnvAccess = (node) => isAstNode(node) &&
...
const quasiText = (template, index) => {
    ...
    rules: {
        "no-direct-api-env": {
            meta: {
                type: "problem",
                messages: {
                    directApiEnv: "Read API deployment bases through '@/lib/api-origins'. The " +
        ...
        "no-raw-api-url": {
            meta: {
                type: "problem",
                messages: {
                    rawApiUrl: "Build API URLs with apiUrl()/browserApiUrl() or call the Eden " +
                        "client. The shared resolver owns the same-origin `/api` prefix.",
                },
            },
            createOnce(context) {
                const checkFirstArg = (args) => {
                ...
                return {
                    ...
                    NewExpression(node) {
                        ...
                        if (isIdentifier(node.callee, "Request") ||
