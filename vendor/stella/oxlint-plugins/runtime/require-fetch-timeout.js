...
const filenameForContext = (context) => context.filename ?? context.getFilename?.() ?? "";
const isStrictFetchWrapperFile = (context) => {
    const normalized = normalizePath(filenameForContext(context));
    const scope = STRICT_FETCH_SCOPES.find((candidate) => candidate.srcPattern.test(normalized));
    if (!scope) {
        return false;
    }
...
const unwrapExpression = (node) => {
    let current = node;
    while (getNodeType(current) === "TSAsExpression" ||
...
const getObjectExpressionProperties = (node) => {
    ...
    if (getNodeType(unwrapped) !== "ObjectExpression" ||
...
const getBindingIdentifierNames = (node) => {
    ...
    if (nodeType === "ArrayPattern" &&
        ...
        return unwrapped.elements.flatMap((element) => getBindingIdentifierNames(element));
    }
...
const isFetchCallee = (callee) => {
    ...
    return (isIdentifier(member.object, "globalThis") ||
...
const isRequestConstructorExpression = (node) => {
    const unwrapped = unwrapExpression(node);
    return (getNodeType(unwrapped) === "NewExpression" &&
        typeof unwrapped === "object" &&
        unwrapped !== null &&
        "callee" in unwrapped &&
    ...
    rules: {
        "require-fetch-timeout": {
            meta: {
                type: "problem",
                messages: {
                    missingSignal: "fetch() must pass `signal` (e.g. " +
                        "`{ signal: AbortSignal.timeout(10_000) }`) so upstream " +
                        "hangs cannot stall the handler.",
                    useFetchWithTimeout: "apps/web/src and apps/api/src must not call fetch() " +
            ...
            createOnce(context) {
                ...
                return {
                    ...
                    CatchClause(node) {
                        pushRequestSignalScope();
                        const param = typeof node === "object" && node !== null && "param" in node
                            ? node.param
                            : null;
                        for (const identifierName of getBindingIdentifierNames(param)) {
                            setRequestSignalState(identifierName, "no");
                    ...
                    AssignmentExpression(node) {
                        ...
                        const nextState = getRequestSignalState(identifierName) === "opaque"
                            ? "opaque"
                            : "no";
                    ...
                    CallExpression(node) {
                        ...
                        if (optionsObjectHasSignal(unwrappedOptions) === "no" &&
