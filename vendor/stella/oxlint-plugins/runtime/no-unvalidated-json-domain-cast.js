...
import { filenameForContext, getPropertyName, isAstNode, isIdentifier, unwrapExpression, } from "./utils.js";
const isProductionBoundaryFile = (filename) => {
...
const isRawJsonType = (node) => {
    ...
    if (isIdentifier(typeNode.typeName, "JsonValue") ||
    ...
    rules: {
        "no-unvalidated-json-domain-cast": {
            meta: {
                type: "problem",
                messages: {
                    unvalidatedDomain: "Raw JSON is not a validated domain value. Keep it as `unknown`/`JsonValue`, then parse only the consumed fields with a runtime schema (use a loose/open schema for evolving upstream payloads).",
                },
            },
            createOnce(context) {
                const jsonParseAliases = new Set();
                const namedTypeAnnotations = new Map();
                const memberAssignmentCandidates = new Array();
                const typedInitializationCandidates = new Array();
                const assertionCandidates = new Array();
                const functionReturnCandidates = new Array();
                const genericCallCandidates = new Array();
                const isRawJsonBoundary = (node) => isJsonParseCall(node, jsonParseAliases) || isResponseJsonCall(node);
                ...
                const hasClosedTypeAnnotation = (identifier) => {
                    const variable = resolveVariable(identifier);
                    return (variable?.defs.some((definition) => isAstNode(definition.name) &&
                        definition.name.typeAnnotation &&
                        !isRawJsonType(definition.name.typeAnnotation)) ?? false);
                };
                const hasRawJsonTypeAnnotation = (variable) => variable.defs.some((definition) => isAstNode(definition.name) &&
                    definition.name.typeAnnotation &&
                    isRawJsonType(definition.name.typeAnnotation));
                const isIdentifierReference = (node) => isIdentifier(node);
                const namedTypeAnnotationFor = (node) => {
                ...
                const hasTypeParameters = (node) => isAstNode(node) &&
                    isAstNode(node.typeParameters) &&
                    Array.isArray(node.typeParameters.params) &&
                    node.typeParameters.params.length > 0;
                const typeDisposition = (node, seenTypeNames = new Set()) => {
                    const typeNode = unwrapTypeAnnotation(node);
                    if (typeNode === null) {
                        return "unresolved";
                    }
                    ...
                    if (typeNode.type === "TSUnionType") {
                        ...
                        const dispositions = types.map((type) => typeDisposition(type, seenTypeNames));
                    ...
                    if (typeNode.type === "TSTypeReference" &&
                        ...
                        if (isAstNode(declaration) &&
                            ...
                            return typeDisposition(declaration.typeAnnotation, nextSeenTypeNames);
                        }
                    }
                    return "closed";
                };
                const memberPropertyTypeAnnotation = (node, propertyName, seenTypeNames = new Set()) => {
                    const typeNode = unwrapTypeAnnotation(node);
                    if (typeNode === null) {
                        return undefined;
                    }
                    if (typeNode.type === "TSTypeReference" &&
                        ...
                        if (isAstNode(declaration) && !hasTypeParameters(declaration)) {
                            const nextSeenTypeNames = new Set(seenTypeNames);
                            nextSeenTypeNames.add(typeName);
                            return memberPropertyTypeAnnotation(declaration.type === "TSTypeAliasDeclaration"
                                ? declaration.typeAnnotation
                                : declaration, propertyName, nextSeenTypeNames);
                        }
                        return undefined;
                    }
                    ...
                    if (Array.isArray(members)) {
                        const property = members.find((member) => isAstNode(member) &&
                    ...
                    if (typeNode.type === "TSInterfaceDeclaration" &&
                        ...
                        for (const heritage of typeNode.extends) {
                            ...
                            const inheritedType = memberPropertyTypeAnnotation(heritage.expression, propertyName, seenTypeNames);
                ...
                const memberPropertyDisposition = (node, propertyName) => {
                    const propertyType = memberPropertyTypeAnnotation(node, propertyName);
                    return propertyType === undefined
                        ? "unresolved"
                        : typeDisposition(propertyType);
                };
                const arrayElementTypeAnnotation = (node, index, seenTypeNames = new Set()) => {
                    const typeNode = unwrapTypeAnnotation(node);
                    if (typeNode === null) {
                        return undefined;
                    }
                    ...
                    const nextSeenTypeNames = new Set(seenTypeNames);
                    nextSeenTypeNames.add(typeName);
                    return arrayElementTypeAnnotation(declaration.typeAnnotation, index, nextSeenTypeNames);
                };
                const memberTargetDisposition = (member) => {
                    ...
                    for (const definition of variable.defs) {
                        ...
                        const disposition = memberPropertyDisposition(definition.name.typeAnnotation, propertyName);
                ...
                const isUnvalidatedJsonValue = (node, seenVariables = new Set()) => {
                    const current = peelRuntimeExpression(node);
                    if (isRawJsonBoundary(current)) {
                        return true;
                    }
                    if (current?.type === "ConditionalExpression") {
                        return (isUnvalidatedJsonValue(current.consequent, seenVariables) ||
                            isUnvalidatedJsonValue(current.alternate, seenVariables));
                    }
                    if (current?.type === "LogicalExpression") {
                        return (isUnvalidatedJsonValue(current.left, seenVariables) ||
                ...
                const hasUnvalidatedJsonAtTypedTarget = (value, typeAnnotation, seenVariables = new Set()) => {
                    const current = peelRuntimeExpression(value);
                    if (current === null) {
                        return false;
                    }
                    if (current.type === "ConditionalExpression") {
                        return (hasUnvalidatedJsonAtTypedTarget(current.consequent, typeAnnotation, seenVariables) ||
                            hasUnvalidatedJsonAtTypedTarget(current.alternate, typeAnnotation, seenVariables));
                    }
                    if (current.type === "LogicalExpression") {
                        return (hasUnvalidatedJsonAtTypedTarget(current.left, typeAnnotation, seenVariables) ||
                            hasUnvalidatedJsonAtTypedTarget(current.right, typeAnnotation, seenVariables));
                    }
                    ...
                    if (current.type === "ObjectExpression") {
                        ...
                        return current.properties.some((property) => {
                            ...
                            if (property.type === "SpreadElement") {
                                return hasUnvalidatedJsonAtTypedTarget(property.argument, typeAnnotation, seenVariables);
                            }
                            ...
                            const propertyType = memberPropertyTypeAnnotation(typeAnnotation, propertyName);
                            return (propertyType !== undefined &&
                                hasUnvalidatedJsonAtTypedTarget(property.value, propertyType, seenVariables));
                        });
                    }
                    if (current.type === "ArrayExpression") {
                        ...
                        return current.elements.some((element, index) => {
                            ...
                            const elementType = arrayElementTypeAnnotation(typeAnnotation, index);
                            if (element.type === "SpreadElement") {
                                return (elementType !== undefined &&
                                    hasUnvalidatedJsonAtTypedTarget(element.argument, elementType, seenVariables));
                            }
                            return (elementType !== undefined &&
                                hasUnvalidatedJsonAtTypedTarget(element, elementType, seenVariables));
                        });
                    }
                    if (!isIdentifierReference(current)) {
                        return false;
                    ...
                    return (variable.defs.some((definition) => definition.type === "Variable" &&
                        definition.node.type === "VariableDeclarator" &&
                        hasUnvalidatedJsonAtTypedTarget(definition.node.init, typeAnnotation, nextSeenVariables)) ||
                        variable.references.some((reference) => reference.isWrite() &&
                            hasUnvalidatedJsonAtTypedTarget(reference.writeExpr, typeAnnotation, nextSeenVariables)));
                };
                const checkAssertion = (node) => {
                    if (!isAstNode(node)) {
                        return;
                    }
                    if (hasUnvalidatedJsonAtTypedTarget(node.expression, node.typeAnnotation)) {
                        context.report({ node, messageId: "unvalidatedDomain" });
                    }
                };
                const containsRawJsonReturn = (node) => {
                    ...
                    return Object.entries(node).some(([key, child]) => key !== "parent" &&
                        key !== "loc" &&
                        key !== "range" &&
                        containsRawJsonReturn(child));
                };
                const checkFunctionReturn = (node) => {
                    if (!isAstNode(node) ||
                        !node.returnType ||
                    ...
                    const returnsRawJson = isUnvalidatedJsonValue(body) || containsRawJsonReturn(body);
                    if (returnsRawJson) {
                        context.report({ node, messageId: "unvalidatedDomain" });
                    }
                };
                return {
                    ...
                    "Program:exit"() {
                        ...
                        for (const node of genericCallCandidates) {
                            ...
                            const typeArgument = isAstNode(node.typeArguments) &&
                        ...
                        for (const declaration of typedInitializationCandidates) {
                            if (isAstNode(declaration.id) &&
                                declaration.id.typeAnnotation &&
                                hasUnvalidatedJsonAtTypedTarget(declaration.init, declaration.id.typeAnnotation)) {
                                context.report({
                                    node: declaration,
