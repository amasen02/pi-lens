...
const isComputed = (node) => getField(node, "computed") === true;
...
const isSafeDbCallee = (callee) => {
    if (isIdentifier(callee, "safeDb")) {
        return true;
    }
    return (getType(callee) === "MemberExpression" &&
        !isComputed(callee) &&
        isIdentifier(getField(callee, "property"), "safeDb"));
};
const isDbAwaitCall = (node) => {
...
const getResultAwaitArgument = (node) => {
    if (getType(node) !== "CallExpression") {
        return null;
    }
    const callee = getField(node, "callee");
    if (getType(callee) !== "MemberExpression" ||
        isComputed(callee) ||
...
const isPromiseAllLikeCall = (node) => {
    if (getType(node) !== "CallExpression") {
        return false;
    }
    const callee = getField(node, "callee");
...
// `Promise.all(...)` / `Promise.allSettled(...)`?
const isPromiseAllMapCallback = (fnNode) => {
...
const isChainRoot = (node) => {
    ...
    return !(getType(parent) === "MemberExpression" &&
        !isComputed(parent) &&
...
const isAwaitArgument = (node) => {
    ...
    return (getType(parent) === "AwaitExpression" &&
        getField(parent, "argument") === current);
};
const matchLocalDeclaration = (stmt, name) => {
...
const resolveLocalFunctionByName = (fromNode, name) => {
...
const containsDbRootedCall = (node, canResolveFurther, viaResolution) => {
    if (Array.isArray(node)) {
        return node.some((child) => containsDbRootedCall(child, canResolveFurther, viaResolution));
    }
    ...
    for (const key of Object.keys(node)) {
        ...
        if (containsDbRootedCall(node[key], canResolveFurther, viaResolution)) {
            return true;
        }
    }
    return false;
...
const isPromiseAllMapFanOutWithDbCallback = (node) => {
...
const findLoopOrMapContext = (awaitNode) => {
    let child = awaitNode;
    ...
    rules: {
        "no-db-await-in-loop": {
            meta: {
                type: "problem",
                messages: {
                    noDbAwaitInLoop: "Database call awaited inside a loop scales the query count " +
            ...
            createOnce(context) {
                const reportAwaitedExpression = (node, argument) => {
                ...
                return {
                    ...
                    YieldExpression(node) {
                        ...
                        const resultAwaitArgument = getResultAwaitArgument(unwrapExpression(getField(node, "argument")));
