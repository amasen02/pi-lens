...
const typeArgumentsOf = (typeNode) => typeNode?.typeArguments?.params ?? typeNode?.typeParameters?.params ?? [];
...
const omittedKeysOf = (typeNode, localTypes, open = new Set()) => {
    ...
    if (typeNode.type === "TSIntersectionType") {
        ...
        return members.flatMap(({ keys }, index) => keys.filter((key) => !members.some((sibling, siblingIndex) => siblingIndex !== index &&
            sibling.suppliers.some((supplier) => !supplier.removed.has(key) &&
                members[index]?.suppliers.some((own) => own.removed.has(key) && own.source === supplier.source)))));
    }
    ...
    if (typeNode.type === "TSUnionType") {
        return intersectAll(typeNode.types
...
const reintroducedKeysOf = (typeNode, localTypes, open = new Set()) => {
    ...
    if (typeNode.type === "TSIntersectionType" ||
        ...
        return typeNode.types.flatMap((member) => reintroducedKeysOf(member, localTypes, open));
    }
    if (typeNode.type === "TSTypeLiteral") {
        return (typeNode.members ?? [])
            .filter((member) => member.type === "TSPropertySignature")
            .map((member) => member.computed === true ? null : getPropertyName(member.key))
    ...
    if (typeNode.type === "TSTypeReference") {
        ...
        if (name === OMIT_TYPE) {
            ...
            const removed = new Set(literalKeysOf(args.at(1)));
            return reintroducedKeysOf(args.at(0), localTypes, open).filter((key) => !removed.has(key));
        }
        if (MODIFIER_UTILITIES.has(name)) {
...
const patternOf = (param) => param?.type === "AssignmentPattern" ? param.left : param;
...
const propsBindingOf = (param) => {
    ...
    const rest = param.properties?.find((property) => property.type === "RestElement");
...
const attributeName = (attribute) => attribute?.type === "JSXAttribute" && attribute.name?.type === "JSXIdentifier"
    ? attribute.name.name
    : null;
...
const hasMeaningfulChildren = (element) => element.children.some(isRenderedChild);
// Wrappers that change a spread argument's type, not its value: the spread is
// runtime-identical to spreading what they wrap.
const unwrapSpread = (node) => node?.type === "TSAsExpression" ||
...
const spreadCanDeliver = (attribute, key) => {
    ...
    return argument.properties.some((property) => property.type !== "Property" ||
    ...
    rules: {
        "no-omitted-prop-respread": {
            meta: {
                type: "problem",
                messages: {
                    pinnedBeforeSpread: "`{{key}}` is omitted from this component's props but written before the props spread, so the spread overwrites it. A value typed wider than the omitting parameter still carries `{{key}}` at runtime — `Omit` narrows what callers may write, not what a spread can deliver. Move the attribute after the last spread.",
                    suppliedBySpread: "`{{key}}` is omitted from this component's props but the props spread can still deliver it: `Omit` narrows what callers may write literally, while a value typed wider stays assignable and keeps the key at runtime. Pin `{{key}}` after the last spread, or suppress with a reason if it is meant to pass through.",
                },
            },
            createOnce(context) {
                ...
                const indexOmittedKeys = () => {
                    for (const { binding, param } of propsParams) {
                        ...
                        const keys = omittedKeysOf(propsType, localTypes).filter((key) => !carried.has(key));
                        if (keys.length > 0) {
                ...
                const reportElement = (node) => {
                    ...
                    for (const key of omitted) {
                        ...
                        const lastOverrideIndex = attributes.findLastIndex((attribute) => attribute.type === "JSXSpreadAttribute" &&
                            spreadCanDeliver(attribute, key));
                        if (lastOverrideIndex === -1) {
                            continue;
                        }
                        const pin = attributes.find((attribute) => attributeName(attribute) === key);
                        if (pin !== undefined &&
                        ...
                        context.report({
                            ...
                            messageId: pin === undefined ? "suppliedBySpread" : "pinnedBeforeSpread",
                            data: { key },
                        });
                    }
                };
                return {
                    ...
                    JSXElement(node) {
                        if (node.openingElement.attributes.some((attribute) => attribute.type === "JSXSpreadAttribute")) {
