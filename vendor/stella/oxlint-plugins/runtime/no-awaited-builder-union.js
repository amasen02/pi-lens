...
const hasFixUnsafeWrapper = (node) => {
    ...
    return (hasFixUnsafeWrapper(expression.consequent) ||
...
const isStrictPrefix = (shorter, longer) => shorter.length < longer.length &&
...
const isBuilderChainUnion = (chains) => chains.some((chain) => chains.some((other) => isStrictPrefix(chain.steps, other.steps) ||
    ...
    rules: {
        "no-awaited-builder-union": {
            meta: {
                ...
                fixable: "code",
                messages: {
                    awaitedBuilderUnion: "Awaiting a ternary over two chain states of `{{root}}` makes " +
                        ...
                        context.report({
                            ...
                            fix: (fixer) => {
                                ...
                                const hasInterveningComment = context.sourceCode
                                    .getAllComments()
                                    .some((comment) => comment.range[0] >= awaitEnd &&
                                ...
                                return [
                                    ...
                                    ...branches.map((branch) => fixer.insertTextBefore(branch, "await ")),
                                ];
                            },
                        });
                    },
