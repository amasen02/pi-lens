...
import { getPropertyName, isAstNode, isIdentifier, unwrapExpression, } from "./utils.js";
...
const isNestedFunction = (candidate, owner) => {
...
const abruptCompletionBypasses = (node, destination) => {
...
const containsBypassingAbruptCompletion = (node, destination, root = node, seen = new Set()) => {
    ...
    for (const [key, value] of Object.entries(node)) {
        ...
        if (Array.isArray(value)) {
            if (value.some((child) => containsBypassingAbruptCompletion(child, destination, root, seen))) {
                return true;
            }
            continue;
        }
...
const isUnconditionallyExecuted = (node, statement) => {
...
const repeatingLoopForNode = (node) => {
    let child = isAstNode(node) ? node : null;
    while (child !== null) {
        const parent = isAstNode(child.parent) ? child.parent : null;
...
const isReturnedCleanupFunction = (functionNode, ownerFunction) => {
...
const functionAlwaysReachesNode = (functionNode, node) => {
    ...
    const revocationIndex = functionNode.body.body.indexOf(revocationSite.statement);
...
const isRevokedByEnclosingFinally = (creation, revocation) => {
    ...
    while (child !== null) {
        ...
        if (parent?.type === "TryStatement" && parent.finalizer === child) {
            ...
            const creationIndex = creationSite.block.body.indexOf(creationSite.statement);
            ...
            return (creationIndex !== -1 &&
                ...
                !creationSite.block.body
                    ...
                    .some((statement) => containsBypassingAbruptCompletion(statement, revocation.node)));
        }
        child = parent;
    }
    return false;
};
const isImmutablePerIterationBinding = (binding, repeatingLoop, creationFunction) => {
    if (!isAstNode(repeatingLoop.body)) {
        return false;
    }
    return (binding?.defs.some((definition) => definition.type === "Variable" &&
        ...
        isDescendantOf(definition.node, repeatingLoop.body)) === true);
};
const revocationCoversCreation = (creation, revocation, callbackRegistrationSites) => {
    const creationFunction = nearestFunction(creation.node);
    const revocationFunction = nearestFunction(revocation.node);
    const usesReturnedCleanup = isNestedFunction(revocationFunction, creationFunction);
    const repeatingLoop = repeatingLoopForNode(creation.node);
    const isReturnedCleanup = usesReturnedCleanup &&
        revocationFunction !== null &&
        isReturnedCleanupFunction(revocationFunction, creationFunction);
    if (repeatingLoop !== null &&
        usesReturnedCleanup &&
        !isReturnedCleanup &&
        callbackRegistrationSites.length > 0 &&
        !isImmutablePerIterationBinding(revocation.argumentBinding, repeatingLoop, creationFunction)) {
        return false;
    }
    ...
    return coverageNodes.some((coverageNode) => {
        ...
        const creationIndex = creationSite.block.body.indexOf(creationSite.statement);
        const revocationIndex = creationSite.block.body.indexOf(revocationSite.statement);
        if (creationIndex === -1 || revocationIndex <= creationIndex) {
            return false;
        }
        return !creationSite.block.body
            .slice(creationIndex + 1, revocationIndex)
            .some((statement) => containsBypassingAbruptCompletion(statement, coverageNode));
    });
};
...
const wrapperCarriesCreatedValue = (parent, current) => {
    ...
    rules: {
        [RULE_NAME]: {
            meta: {
                type: "problem",
                messages: {
                    noObjectUrlLeak: "This URL.createObjectURL() result is not provably revoked. " +
            ...
            createOnce(context) {
                ...
                const callbackRegistrations = new Map();
                ...
                const canonicalBindingInfo = (node, visited = new Set()) => {
                    const expression = unwrapExpression(node);
                    ...
                    for (const definition of variable.defs) {
                        ...
                        const aliasedBinding = canonicalBindingInfo(definition.node.init, visited);
                ...
                const canonicalBinding = (node) => canonicalBindingInfo(node)?.binding ?? null;
                ...
                const cleanupFunctionBinding = (node) => {
                    if (node === null) {
                        return null;
                    }
                ...
                const recordTimerCallbackRegistration = (node) => {
                    const callee = unwrapExpression(node.callee);
                    if (!isIdentifier(callee, "setTimeout") ||
                    ...
                    const registrations = callbackRegistrations.get(callbackBinding) ?? [];
                    registrations.push(node);
                    callbackRegistrations.set(callbackBinding, registrations);
                };
                const cleanupCallbackRegistrations = (node) => {
                ...
                const hasInterveningAssignment = (binding, creationPosition, revocationPosition) => (assignments.get(binding) ?? []).some((position) => position > creationPosition && position < revocationPosition);
                return {
                    before() {
                        creations.length = 0;
                    ...
                    CallExpression(node) {
                        ...
                        if (isUrlMethodCall(node, "createObjectURL")) {
                            ...
                            creations.push({
                                binding: owner === null ? null : resolveVariable(owner.identifier),
                                node,
                    ...
                    "Program:exit"() {
                        ...
                        for (const creation of creations) {
                            const directlyRevoked = revocations.some(({ revokedCreation }) => revokedCreation === creation.node);
                            if (directlyRevoked) {
                                continue;
                            }
                            if (creation.binding !== null) {
                                ...
                                const matchingRevocation = revocations.find((revocation) => {
                                    ...
                                    if (revocation.aliasPositions.some((position) => position < creation.position)) {
                                        return false;
                                    }
                                    return ((revocation.position > creation.position ||
                                        isNestedFunction(nearestFunction(revocation.node), nearestFunction(creation.node))) &&
                                        revocationCoversCreation(creation, revocation, cleanupCallbackRegistrations(revocation.node)) &&
                                        !hasInterveningAssignment(binding, creation.position, revocation.position));
                                });
