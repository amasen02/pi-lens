...
import { getPropertyName, isCallTo, isIdentifier, isMemberAccess, } from "./utils.js";
...
const isStringErrorCall = (node) => isCallTo(node, "String") && node.arguments.some(isRawErrorExpression);
const isRawErrorMember = (node) => {
    ...
    return (isIdentifier(node.object) && RAW_ERROR_IDENTIFIERS.has(node.object.name));
};
const isRawErrorExpression = (node) => {
    ...
    if (node.type === "ConditionalExpression") {
        return (isRawErrorExpression(node.consequent) ||
    ...
    rules: {
        "no-raw-error-logging": {
            meta: {
                type: "problem",
                messages: {
                    rawErrorAttribute: "Do not log raw '{{name}}' values. Log a structural error tag with errorTag(error) as 'error.type' instead.",
                    rawErrorStderr: "Do not write raw error messages, stacks, causes, or String(error) to stderr. Use the error class/tag instead.",
                },
            },
            createOnce(context) {
                return {
