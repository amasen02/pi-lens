...
pi-lens bundles **467 enabled** ast-grep rules (283 pi-lens-authored + 184 vendored from CodeRabbit) across **16 languages**, plus 16 disabled.
...
| `no-raw-use-effect` | error | no | pi-lens | Use the owned lifecycle primitive instead of useEffect |
...
| `no-unsafe-inner-html` | error | no | pi-lens | Dynamic HTML requires an adjacent safe-html provenance marker |
...
| `no-async-context-enter-with` | error | no | pi-lens | Use AsyncLocalStorage.run() instead of enterWith() |
...
| `no-crypto-random-uuid` | error | no | pi-lens | Use the owned UUID generator |
...
| `no-detached-void` | error | no | pi-lens | Do not detach promises with void; use detached() |
...
| `no-eager-singleton` | error | no | pi-lens | Construct side-effecting clients lazily |
...
| `no-nanoid` | error | no | pi-lens | Do not reintroduce Nano ID |
...
| `no-path-prefix-containment` | error | no | pi-lens | Use path-aware containment instead of a bare prefix check |
...
| `no-static-devtools-import` | error | no | pi-lens | Do not statically import development-only tooling |
| `no-swallowed-rejection` | error | no | pi-lens | Do not convert a rejected promise into a constant empty value |
...
| `no-unjustified-double-assertion` | error | no | pi-lens | Explain a double assertion with a nearby SAFETY comment |
...
| `no-vacuous-throw-assertion` | error | no | pi-lens | Name the expected error in toThrow() |
