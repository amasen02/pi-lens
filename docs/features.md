...
pi-lens takes a bounded snapshot around a call that names a file, replays
whatever actually changed through the same chain, then remembers that tool as
mutating — for the session on the first sighting, and on disk under the
project's data directory on the second, so later sessions classify it by name
with no snapshot at all. A tool that names no file is caught at `agent_settled`
by a hash check over the files pi-lens has already read, written, diagnosed or
opened on a language server; a file it has never seen has no baseline and is
therefore not covered (`clients/observed-mutation.ts`).
