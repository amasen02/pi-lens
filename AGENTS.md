...
sequence hash, and provider/model identity on both adjacent usage records
before claiming either. Measure the gap to REQUEST time, never to the response
...
- `cache_usage` heuristic causes have a three-part evidence gate: request-time correlation, a non-truncated bounded sequence observation, and provider/model identity on both adjacent usage records. Missing any part yields its specific unknown reason before TTL or partial-eviction is considered; direct observed prefix or identity changes still outrank the gate. (#1996 review)
