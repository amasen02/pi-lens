...
- **`config_resolved` is the config stack's POSITIVE record (#2526).** The Phase 0 config stack (#2429/#2440/#2456/#2513) used to prove itself only by the ABSENCE of `PILENS_CFG_*` / `config-ignored` rows — correct for a canonical-only layout, and indistinguishable from "resolution never ran". `loadLSPConfig` — the ONE funnel every production resolution goes through (`initLSPConfig` at session start and per served root, the warm MCP boot's `ensureLspConfig`, `ensureLSPConfigInitialized` on the first edit) — now writes one `config_resolved` phase per session carrying `documents[]` (`{tier, file, legacy}`), `countsByTier`, `recordCount`, `deniedServers` and `resolveMs`, plus a matching `config resolved …` line in `sessionstart.log`. **Redacted structurally, never by mode:** the payload comes from `summarizeConfigResolution` (`clients/config-resolve.ts`), the same projection `pilens_effective_config` embeds for its `documents`/`provenanceCounts` — one definition, so the on-demand answer and the session record cannot drift. It is a projection of a resolution the caller ALREADY performed (#2513's facade rule: no second `resolvePiLensConfig`), and both writes are synchronous, so the session-start hook path gains nothing to await. Once per session via `claimPhaseOncePerSession()` — the logger's own session-scoped claim set, re-armed by `resetOncePerSessionPhases()` in `handleSessionStart` (catalog shape 17, registered in `tests/support/session-state-registry.ts`); without the re-arm the record would fire once per PROCESS. The claim lives in `latency-logger.ts` rather than in the producer so "one row per session" is a property of the logger, reusable by the next session-fact row, instead of a latch each producer re-implements. `npm run logs:smells`'s `config-resolution` smell reads it: session starts with no row (a deficit against `session_start cwd:` lines), or a legacy document that produced zero migration records.
