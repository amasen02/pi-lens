...
**Deferred off-hook LSP work (#2504).** Moving the cold fresh-pull loop off the awaited hook creates work that outlives the turn that armed it, which is its own defect surface. Two rules. **Bounds:** the turn's `ctx.signal` is USELESS to it — `index.ts` clears the ambient slot in its `finally`, so a captured turn signal can never fire — and a wall deadline checked between files leaves a wedged `getDiagnostics` unbounded. `clients/deferred-lsp-work.ts` (a declared zero-dependency leaf, so `clients/lsp/index.ts` gains no import edge to the warning pipeline) owns ONE handle and ONE abort signal; `resetLSPService` fires it, before any teardown and before its no-live-service early return, which is how `session_shutdown`, `session_start` and the LSP idle reset all reach the loop through the single seam they already share. Arming aborts whatever held the slot, every round trip races a per-call timeout against that signal, the service is resolved ONCE per loop (re-resolving `getLSPService()` per file lets the loop re-spawn a server the idle reset just retired), no file is opened once the signal is up, and an aborted loop publishes nothing. **Ordering:** a deferred report carries the ORIGINATING turn's `turnIndex`/`projectSeq` and may land many turns later, so it is published through `writeDeferredActionableWarningsReport`, which declines when the runtime's `projectSeq` has moved past the report or anything newer is already persisted. Writing it unconditionally overwrote turn N+1's report, `agent_end` then read `project_seq_mismatch` and silently skipped its autofix pass, and `lens_diagnostics` re-served the stale delta.
