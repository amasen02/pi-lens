...
**`clients/gradle-ktfmt-style.ts` carries the Gradle `ktfmt { }` extension block's style selection into ktfmt's CLI argv (#2468).** ktfmt's own CLI never reads `build.gradle` — style is `--google-style`/`--kotlinlang-style`-flag-only (verified against ktfmt v0.63's `ParsedArgs.kt`, the version `clients/installer/index.ts` pins; the default is `--meta-style`, matching pi-lens's pre-fix bare invocation). `resolveKtfmtGradleStyle` walks up from the formatted file via the shared `findNearestMarkerRoot` (home-ceiling guarded, `homeDir`-injectable for tests, same primitive `resolveCargoPackageEdition` uses) to the nearest `build.gradle(.kts)`/`settings.gradle(.kts)`, and reuses `clients/tool-policy.ts`'s `stripGradleCommentsAndStrings`/`namedGradleBlockBodies` lexical pre-pass (now exported for this reuse) to read the `ktfmt { }` block body instead of a second hand-rolled Gradle parser — do not add a THIRD Gradle-block reader; extend this one or `getSpotlessKotlinFormatter`'s. The walk stops at the FIRST directory with a gradle file, so a nested module's own declaration always wins over an ancestor's — it never climbs past a gradle file that exists but declares no recognized style. `dropboxStyle()` — removed from the `com.ncorti.ktfmt.gradle` plugin in 0.19.0 (2024-07-03, "no longer supported by ktfmt") and never a ktfmt CLI flag at any version — is a recognized-but-unsupported selection: logged via `logExtension` and treated as "no usable style," never guessed into a nonexistent `--dropbox-style` flag. `ktfmtFormatter.resolveCommand` carries the resolved flag ahead of the file path in argv (`[binary, "--google-style"|"--kotlinlang-style", filePath]`); any miss (no gradle file, no `ktfmt { }` block, no recognized style, or the unsupported one) falls back to the pre-#2468 bare command unchanged.
