...
- **Tool-resolution bin walkers stop at HOME by default; config-file walkers keep reading past it (#2514/#2517).** `findLocalBinUpwards` (`package-manager.ts` — the `node_modules/.bin/<tool>` climb also reused by stylua/taplo/knip/jscpd and, via `findNodeToolBinary`, madge) and `clients/formatters.ts`'s `findInVendorBin`/`findInVenv` (`vendor/bin`/`.venv`|`venv` bin climbs) are DEFAULT-ON `isAtOrAboveHomeDir`-ceilinged (`homeDir` injectable, default `os.homedir()`) — a bin match found at or above `$HOME` can never be the current project's own installed dependency, so escaping the project here means STOP. This is the opposite default from `findLocalToolConfig` (#2472 review round 3, F1), which is UNCEILINGED by default because a config file living directly at `$HOME` (`~/typos.toml`, `~/sgconfig.yml`, …) is the user's legitimate global config that the underlying tool would read on its own — config-file lookups keep reading past HOME, tool-resolution (bin) lookups stop. `clients/formatters.ts`'s `findInNodeModules` used to be a private, byte-for-byte duplicate of `findLocalBinUpwards`'s pre-fix unceilinged loop (a stray `~/node_modules/.bin/oxfmt.cmd`, e.g. from a home-level pi-extensions manifest, was climbed to and used as "the project's" formatter) — it now delegates to the shared walker instead of carrying a second copy of the ceiling fix. `#2517` tracks building the single per-walker escape-policy table (stop vs keep-reading) this bullet and `findLocalToolConfig`'s doc comment currently each state independently.
