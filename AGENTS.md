...
**`getGlobalPiLensDir()` redirects for a probe/worktree cwd with no `PI_LENS_HOME` pin (#2506).** vitest is hermetic (the pin above), but an ad-hoc probe against the BUILT `clients/*.js` — a bare `node -e`, a throwaway `.mjs`, a harness script run OUTSIDE vitest — has no test-mode gate and no home pin at all, so every logger/ledger/cache it touches used to fall straight through to the real `~/.pi-lens`. Confirmed live on 2026-09-02: two review probes run this way left 42 rows of fixture garbage in the maintainer's real telemetry, root-caused as short-lived PROBE processes (not the vitest-hermetic writer the round-1 fix on this issue mistakenly targeted — that fix was reverted). The actual fix is entirely in `getGlobalPiLensDir()` itself, the one resolver every global writer already routes through: when `PI_LENS_HOME` is unset, `isTestMode()` (`clients/env-utils.ts`) is false, and `process.cwd()` sits under a `.claude/worktrees/` segment or under `os.tmpdir()` — the two shapes an agent worktree or a throwaway probe script actually runs from — every writer is redirected to `<cwd>/.pi-lens-probe-home` instead, never the real home directory; `PILENS_PROBE=1` forces the same redirect for a probe run from an ordinary project checkout. The first redirect in a process logs one stderr line and records a `global-dir-probe-redirect` degradation — routed through `clients/probe-home-state.ts`, a true zero-import dependency leaf (same shape as `process-singletons.ts`), never a direct import of `degradation-ledger.ts` in either direction: `file-utils.ts` and `degradation-ledger.ts` are already mutually reachable through the existing `extension-log.ts`/`latency-logger.ts`/`safe-spawn.js` cycle (`.dependency-cruiser-known-violations.json`), so ANY new edge between them — a dynamic `import()` included, since `no-client-cycles`'s dynamic-import exemption only excuses that ONE edge from being flagged, not from closing the cycle other static edges on it then also get flagged on — adds new, unpinned violations (caught live: a first attempt using a dynamic import failed CI's "Dependency boundaries" gate with exactly this shape). `file-utils.ts` writes to the leaf; `degradation-ledger.ts` reads it back at summary time (`getDegradationSummary()`), the same read-time-fold inversion `log-sink-write-failure`/`process-singleton-reset` already use. `getProjectDataDir(cwd)`'s default branch composes through `getGlobalPiLensDir()`, so it inherits the same redirect for free — no separate change needed there. See `.claude/agents/pi-lens-fixer.md`'s "Probe hygiene (mandatory)" section for the companion rule: pin `PI_LENS_HOME` yourself before any such probe rather than relying on this fallback, which exists to bound the blast radius of forgetting, not to replace the pin.
